  ...
  private async reportResult(sessionId: string): Promise<void> {
    ...
    await this.prompt(
      ...
      [
        ...
        "",
        "Any figure or fact above is settled — the session already computed or found it. State it",
        "as given. Do not recompute it, re-derive it, or run the numbers again yourself to check;",
        "that is exactly the mistake of trusting your own arithmetic over a real result, in the",
        "one place it is hardest to notice, because the number is sitting right there correctly and",
        "the only way to get it wrong is to redo work that is already done.",
        "</portal-result>",
      ].join("\n"),
      { internal: true },
    ).catch(() => {
