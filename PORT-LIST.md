...
| BC-49 | Progressive disclosure: find the *exact* section matching this item before falling back to the tail of the file. | `neededSections()` in `pi/step-runner.ts`. BirdClaw finds it with a regex over headings and definitions; this does not have to, because `write_next` records where every section landed, so the text is sliced exactly. A step gets an earlier section when its own description names it — which is what "uses all three" looks like once the plan is written down. Not every earlier section: that is the accumulating context this exists to avoid, reached by another road. | done |
